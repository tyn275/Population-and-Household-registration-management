...
function setGreetingSidebar(){
  ...
  $('#sbDate').textContent = `${now.toLocaleTimeString('vi-VN',{hour:'2-digit',minute:'2-digit'})} • ${now.toLocaleDateString('vi-VN')}`;
}
let _clock;
function startClock(){
...
const fallbackMe = {
  ...
  tempStay:[
    { tu_ngay:'2025-08-01', den_ngay:'2026-02-01', noi:'KTX Bách Khoa', ly_do:'Học tập' }
  ],
  tempAbs:[],
  history:[
    { date:'2025-01-12', type:'CHUYEN_DEN', note:'Chuyển đến từ P. Trung Văn' },
...
const fallbackNews = [
  ...
  { id:2, title:'Thông báo cắt điện bảo trì 19:00–21:00 ngày 6/11', date:'2025-11-04' },
...
const fallbackTickets = [
  {
    ...
    date:'2025-10-25',
    category:'HA_TANG',
    content:'Đèn đường trước số 15 bị hỏng 3 ngày nay, khu vực tối và nguy hiểm.',
...
const fmtGender=(g)=> (g===1||g==='1'||g==='NAM')?'Nam':(g===2||g==='2'||g==='NU'||g==='NỮ')?'Nữ':'Khác';
const fmtStatus=(s)=>({THUONG_TRU:'Thường trú',TAM_TRU:'Tạm trú',TAM_VANG:'Tạm vắng'})[s]||s||'—';
const maskCCCD=(v)=>{
  if(!v) return '—';
  const s=String(v);
...
function renderLeader(leader){
  ...
  $('#leaderInfo').textContent = `SĐT: ${leader?.phone||'—'} • Email: ${leader?.email||'—'}`;
...
function renderCitizen(c){
  ...
  $('#citPermRegDate').textContent = fmtDate(cit.ngay_dang_ky_thuong_tru || cit.ngay_thang_dang_ki_thuong_tru);
  $('#citAddress').textContent = cit.thuong_tru || cit.dia_chi_thuong_tru || '—';
...
function renderTemp(tt=[], tv=[]){
  $('#ttList').innerHTML = (tt||[]).map(r =>
    `<div class="item">Từ ${fmtDate(r.tu_ngay)} đến ${fmtDate(r.den_ngay)} – ${r.noi||r.noi_tam_tru||''} (${r.ly_do||''})</div>`
  ).join('') || '—';
  $('#tvList').innerHTML = (tv||[]).map(r =>
    `<div class="item">Từ ${fmtDate(r.tu_ngay)} đến ${fmtDate(r.den_ngay)} – ${r.noi_den||r.dest||''} (${r.ly_do||r.reason||''})</div>`
  ).join('') || '—';
}

function renderHistory(arr=[]){
  $('#historyList').innerHTML = (arr||[]).map(e =>
    `<div class="ev"><b>${fmtDate(e.date)}</b> • ${e.type} – ${e.note||''}</div>`
  ).join('') || '—';
}
...
function renderNews(list=[]){
  ...
  box.innerHTML = (list||[]).map(n => `
    ...
    </div>`).join('') || '<div class="muted">Hiện chưa có thông báo.</div>';
}

function statusBadge(s){
  const map = { HOAN_TAT:'ok', DANG_XU_LY:'proc', DA_TIEP_NHAN:'new' };
  const label = { HOAN_TAT:'Hoàn tất', DANG_XU_LY:'Đang xử lý', DA_TIEP_NHAN:'Đã tiếp nhận' }[s] || s;
...
function renderTickets(list){
  ...
  box.innerHTML = data.map(t => {
    ...
    const content = t.content || t.noi_dung || '(Chưa có nội dung chi tiết để hiển thị.)';
    ...
    return `
      <div class="ticket">
        ...
        <div class="ticket__body" hidden>
          ...
          ${lastNote ? `<div style="margin-top:4px;"><b>Ghi chú của Tổ trưởng:</b> ${lastNote}</div>` : ''}
        </div>
      </div>`;
  }).join('');
...
function renderKPI(me){
  ...
  $('#kpiStatus').textContent = fmtStatus(me.citizen?.trang_thai || me.nhan_khau?.trang_thai || '—');

  const data = [5,7,6,9,11,10,12,13,12,14,15,16];
  const w=180,h=34,max=Math.max(...data),min=Math.min(...data);
  const pts = data.map((v,i)=>`${(i/(data.length-1))*w},${h-((v-min)/(max-min))*h}`).join(' ');
  $('#sparkStay').innerHTML = `<svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none">
...
function navigateToSection(id){
  $$('[data-section]').forEach(sec => sec.hidden = (sec.id !== id.replace('#','')));
...
// Meeting modal
function openMeet(){ $('#meetModal').classList.remove('hide'); }
function closeMeet(){ $('#meetModal').classList.add('hide'); }
function bindMeet(){
...
function openMemberModal(m){
  ...
  $('#memAddress').textContent = detail.thuong_tru || detail.dia_chi || detail.noi_o || '—';
...
window.addEventListener('DOMContentLoaded', async ()=>{
  ...
  const [me, news, tickets] = await Promise.all([loadMe(), loadNews(), loadMyTickets()]);
  currentMe = me;
